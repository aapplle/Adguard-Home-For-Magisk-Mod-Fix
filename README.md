# AdGuard Home For Android — 软重启/Late-load 竞态修复版（minfix）

[liuzq2002/Adguard-Home-For-Magisk-Mod](https://github.com/liuzq2002/Adguard-Home-For-Magisk-Mod)
的最小 diff 修复 fork：解决 KernelSU/Magisk **软重启**以及 **KSU late-load 多阶段触发**
下的 DNS 去广告模块竞态问题。

- 当前 main：**v20260720-minfix9**
...
| KSU late-load 与 soft reboot 重复执行 service.sh | late-load 首次跳过（boot_id marker），后续 soft reboot 正常启动 |
| AGH 进程存活但 DNS 端口未监听 | 看门狗循环补 `port_listening`，自动清空 REDIRECT 让 DNS 直通 |
...
- 流水线：拉上游 main 源码 → `fixes/apply-fixes.sh` 套用修复 → 语法与标记
  ...
  文件）→ 合并打包 zip → 创建 Release + 更新 Update.json。
...
→ soft reboot → 清理→成功
→ soft reboot → 健康跳过
→ 重启后重新提权 → late-load 首次跳过
→ soft reboot → 清理→成功
```

日志中不应再出现「另一实例已完成启动，本实例跳过」（fix8 双跑合并已移除）。
